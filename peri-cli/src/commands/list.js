...
export async function listVersions() {
  ...
  try {
    ...
    formattedReleases.forEach((release, index) => {
      const isCurrent = release.tag === currentVersion ? ' (current)' : '';
      const isLatest = index === 0 ? ' (latest)' : '';

      console.log(`  ${index + 1}. ${release.tag}${isLatest}${isCurrent}`);
      ...
      if (platformBinary) {
        console.log(`     Binary: ${platformBinary.name} (${platformBinary.size})`);
      } else {
        console.log(`     Binary: Not available for ${platformInfo.target}`);
      }
