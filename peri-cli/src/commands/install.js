...
import { getPlatformInfo, getInstallDir, getExecutablePath, setCurrentVersion } from '../utils/config.js';
import { getLatestRelease, getReleaseByVersion, findAssetForPlatform } from '../utils/github.js';
...
export async function install(options) {
  ...
  try {
    ...
    if (!asset) {
      console.error(`❌ No binary found for platform: ${platformInfo.target}`);
    ...
    // 下载二进制文件
    const targetPath = path.join(versionDir, platformInfo.isWindows ? 'agent-tui.exe' : 'agent-tui');
