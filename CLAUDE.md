...
| `/relay` | 打开远程控制配置面板（配置 URL/Token/Name，写入 `~/.zen-code/settings.json`） |
...
| 参数 | 说明 |
|------|------|
| `-y, --yolo` | 跳过 HITL 审批（等同于 `YOLO_MODE=true`） |
| `--remote-control [url]` | 连接 Relay Server（无参数时从 `remote_control.url` 配置读取） |
| `--relay-token <token>` | Relay 认证 Token（可选，优先级低于配置文件） |
| `--relay-name <name>` | 客户端名称（可选） |
...
- **新增弹窗面板**：`Event::Paste` 独立于 key event 链，必须在该分支单独拦截并路由到 `panel.paste_text()`；`Ctrl+V` 需在 `handle_xxx_panel` 内单独处理（主循环的 Ctrl+V 不会执行）。
- **EditField 导航**：`next()/prev()` 链必须与表单实际渲染字段一致，不渲染的字段要从导航链跳过。
