...
impl App {
    ...
    pub async fn check_relay_reconnect(&mut self) {
        ...
        let Some((url, token, name, user_id)) = self.relay.relay_params.clone() else {
            return;
        };
        ...
        match rust_relay_server::client::RelayClient::connect(&url, &token, name.as_deref(), &user_id).await {
            ...
            Err(_) => {
                ...
                self.relay.relay_reconnect_at = Some(
    ...
    pub fn poll_relay(&mut self) -> bool {
        ...
        if let Some(rx) = self.relay.relay_event_rx.as_mut() {
            loop {
                match rx.try_recv() {
        ...
        if disconnected {
            ...
            self.relay.relay_event_rx = None;
            self.relay.relay_client = None;
            ...
            let _ = self.core.render_tx.send(RenderEvent::AddMessage(vm));
            // 有缓存参数时，3 秒后自动重连
            if self.relay.relay_params.is_some() {
                self.relay.relay_reconnect_at = Some(
                    std::time::Instant::now() + std::time::Duration::from_secs(3),
                );
            }
            ...
            match web_msg {
                WebMessage::UserInput { text } => {
                    ...
                    if self.core.loading {
                        self.core.pending_messages.push(text);
                    } else {
                        self.submit_message(text);
                    }
                }
                WebMessage::HitlDecision { decisions } => {
                    if let Some(InteractionPrompt::Approval(prompt)) = self.agent.interaction_prompt.take() {
                ...
                WebMessage::AskUserResponse { answers } => {
                    if let Some(InteractionPrompt::Questions(prompt)) = self.agent.interaction_prompt.as_mut() {
